'use client'
import { RotateCcw, Target } from '@/components/icons';
;
...
export function StrReview({ onBack }: { onBack: () => void }) {
  const { evaluation, resetSimulation } = useStrTriageStore();
  ...
  const totalTerms = evaluation.actionEvaluations.length;
  const accuracy = Math.round((correctCount / totalTerms) * 100);

  // Calculate average ACoS improvement
  const acosBefore = evaluation.metricsBefore.acos;
  const acosAfter = evaluation.metricsAfter.acos;
  const acosImprovement = isFinite(acosBefore) && isFinite(acosAfter)
    ? ((acosBefore - acosAfter) * 100).toFixed(1)
    : '0.0';
    ...
    <div className="space-y-6 max-w-xl mx-auto">
      ...
      >
        <Card className="border-rose-500/20 bg-gradient-to-br from-rose-500/5 via-card to-card overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-rose-500/5 rounded-full -translate-y-1/2 translate-x-1/3 blur-3xl" />

          <CardContent className="relative py-8 flex flex-col items-center gap-6">
            <div className="flex items-center gap-2">
              <Icon name="trophy" className="h-6 w-6 text-rose-400" />
      ...
      >
        <Card>
          <CardContent className="py-4 flex flex-col items-center gap-1">
            <Target className="h-4 w-4 text-rose-400 mb-1" />
        ...
        <Card>
          <CardContent className="py-4 flex flex-col items-center gap-1">
            ...
            <p className="text-xs text-muted-foreground">Terms Correct</p>
          </CardContent>
        </Card>

        <Card>
          <CardContent className="py-4 flex flex-col items-center gap-1">
            <Icon name="trend-up" className="h-4 w-4 text-sky-400 mb-1" />
            <p className="text-2xl font-bold font-mono text-emerald-400">
              {acosImprovement}%
            </p>
            <p className="text-xs text-muted-foreground">ACoS Improvement</p>
          </CardContent>
        </Card>
